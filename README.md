...
Stock Monitor 是一个跨平台行情桌面应用，用 Electron + React + MobX MVVM 实现远端行情查询、多源切换、当日分时、K 线展示、自选股管理、历史 K 线缓存、K 线历史回测、做T盈亏测算和手动 AI 分析。
...
- AI 分析：顶部 `AI` 入口打开手动分析弹窗，顶部 `更多 -> AI 设置` 配置 HTTP provider；支持自然语言生成策略、回测报告解读、策略诊断、参数优化助手、自然语言智能选股、策略多周期/多股票对比、市场环境识别、每日复盘、新闻/公告 + K 线联合分析和实验性涨跌预测
...
- 顶部工具栏：证券代码、自选、分时/K线、K 线周期/复权/日期范围、刷新、指标、做T、AI、当前证券标题和 `更多` 菜单；`更多` 聚合数据源、代理、AI 设置、本地缓存导入导出、检查更新和启动检查更新，分时视图隐藏 K 线专属控件
