...
- K 线缓存：自选股栏 `缓存` 入口打开历史 K 线缓存弹窗，支持按自选股、周期、复权和日期范围查看完整性、手动刷新、取消任务和清理选中缓存
- K 线历史回测：顶部 `策略` 入口打开回测面板，支持日线/周线/月线下比较均线交叉、突破回撤、RSI 超买超卖和 MACD 趋势确认等候选策略
...
自选股栏支持单只添加、批量粘贴、管理模式删除所选和点击切换证券；K 线模式下还可以从自选股栏进入历史 K 线缓存管理。

<img src="docs/assets/screenshots/kline-cache-management.png" alt="历史 K 线缓存" width="900">

历史 K 线缓存弹窗按自选股、周期、复权和日期范围展开组合，展示缓存状态、记录数、缓存范围、缺失范围和任务进度，并支持刷新全部、刷新选中、清理选中和取消任务。

<img src="docs/assets/screenshots/kline-strategy-backtesting.png" alt="K 线历史回测" width="900">

K 线历史回测面板用于比较候选策略在所选历史区间内的程序化信号和历史表现排名。历史回测不代表未来表现，也不构成买卖建议或收益承诺。
...
在自选股栏点击 `缓存` 会打开历史 K 线缓存弹窗。缓存管理用于提前准备自选股历史 K 线数据，主要能力包括：

- 按当前 K 线数据源、多个周期、多个复权口径和日期范围查询缓存完整性。
- 按 `自选股 × 周期 × 复权` 展开状态行，展示记录数、缓存范围、缺失范围和最近刷新时间。
- 手动刷新全部组合或刷新选中组合，任务运行时展示进度并支持取消剩余任务。
...
顶部工具栏 `策略` 按钮会打开 K 线历史回测面板，用于比较候选策略在所选历史区间内的程序化信号和历史表现。

第一版能力：

- 支持日线、周线和月线；暂不支持 5/15/30/60 分钟 K 线回测。
- 内置均线交叉、突破回撤、RSI 超买超卖和 MACD 趋势确认四类候选策略模板。
- 支持设置回测开始日期、结束日期、初始资金、费用率和滑点率。
- 优先读取本地历史 K 线缓存；缓存缺失或不完整时，会通过既有缓存刷新链路准备数据。
- 展示历史表现排名、收益和回撤指标、交易明细、信号列表、回测区间、实际数据区间和回测假设。
- 选中策略后，K 线图表会展示该策略的历史买入/卖出信号标记。
...
- `workspace.klineStrategySettings`：K 线策略模板选择、参数、回测区间和回测假设
...
- K 线策略模板、参数校验、历史信号生成、交易撮合、收益/回撤指标、策略排名和 ViewModel 回测编排
