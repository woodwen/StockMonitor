...
- 顶部工具栏：证券代码、自选、分时/K线、K 线周期/复权/日期范围、刷新、指标、做T、当前证券标题和 `更多` 菜单；`更多` 聚合数据源、代理、本地缓存导入导出、检查更新和启动检查更新，分时视图隐藏 K 线专属控件
- 帮助菜单：使用说明书、版本更新说明、检查更新、关于 Stock Monitor
- 状态栏：当前视图的数据源、证券代码、记录数或分时点数、最新行情、加载状态、更新状态
- 本地持久化：查询条件、指标开关与参数、策略偏好、自选股、做T测算、网络代理、启动检查更新配置会保存到 `electron-store`；历史 K 线缓存保存到本地应用数据目录，并可通过 JSON 备份文件导入导出
