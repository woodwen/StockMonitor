...
Stock Monitor 是一个跨平台行情桌面应用，用 Electron + React + MobX MVVM 实现远端行情查询、多源切换、当日分时、K 线展示、自选股管理、历史 K 线缓存、K 线历史回测、做T盈亏测算和手动 AI 分析（测试中）。
...
AI connector 默认禁用。用户可以自行配置 OpenAI-compatible HTTP provider（OpenAI-compatible、DeepSeek、MiniMax、智谱 GLM、通义千问/DashScope、Kimi/Moonshot、硅基流动/SiliconFlow、百川智能/Baichuan、火山方舟/Ark、自定义 provider），再手动把当前工作区摘要发送给 AI。HTTP API key 只在 Electron main process 凭据仓库处理，renderer、日志和本地备份不包含明文密钥、加密密文或 Authorization header。

## 免责声明

本项目仅用于个人学习、技术研究和行情展示验证，不构成任何投资建议、交易建议、荐股服务或收益承诺。应用内行情数据来自公开网页接口或第三方服务，可能存在延迟、缺失、错误、接口变更、访问受限或服务不可用等情况；所有技术指标、策略信号、策略回测和做T测算仅为程序化计算或历史估算结果，不保证准确性、完整性或适用于任何交易决策。用户应自行核验数据并独立承担投资和使用风险。
...
```ts
{
  ...
  indicatorSettings: createDefaultIndicatorSettings(), // K 线：BOLL、VOL、策略信号默认开启
...
保存过工作区设置后，每次重启仍会把 K 线日期范围的结束日期滚动到启动当天，并按原日期窗口长度同步平移开始日期；在当前会话中手动选择的历史日期范围会继续按用户设置使用。
...
- 市场范围：沪深京股票、ETF、指数
- 默认查询：分时视图、东方财富分时源、`sh000001`；K 线保留东方财富、日线、前复权、近 2 年，重启时日期基准自动滚动到当天
- 数据源拆分：K 线使用 `workspace.query.sourceId`，分时使用 `workspace.timeshareSourceId`，两者共享证券代码但互不覆盖
- 分时：价格线、均价线、昨收参考线、右侧涨跌幅轴、成交量柱、主图/成交量/副图指标、分区图例、十字线、tooltip，并压缩午间休市空档
...
- 数据源管理：顶部 `更多 -> 数据源` 打开弹窗，按当前视图测试数据源请求状态、耗时和返回记录数，并展示分时能力；分时视图只允许切换到支持分时的数据源
...
- K 线历史回测：顶部 `策略` 入口打开回测面板，默认选择全部候选策略模板，支持日线/周线/月线下比较均线交叉、突破回撤、RSI 超买超卖、MACD 趋势确认以及均线多头排列、N 日新高突破、放量突破、布林带突破、布林带均值回归、KDJ 超卖反弹、ATR 趋势跟踪、缩量回踩均线等候选策略
- 做T盈亏测算：顶部 `做T` 入口打开右侧测算 Drawer，支持费用明细、ETF 免印花税、多笔记录、总盈亏和本地持久化
- AI 分析（测试中）：顶部 `AI（测试中）` 入口打开手动分析弹窗，顶部 `更多 -> AI 设置` 配置 HTTP provider；支持自然语言生成策略、回测报告解读、策略诊断、参数优化助手、自然语言智能选股、策略多周期/多股票对比、市场环境识别、每日复盘、新闻/公告 + K 线联合分析和实验性涨跌预测
- 周期：日线、周线、月线、5/15/30/60 分钟
- 复权：不复权、前复权、后复权；不支持复权的数据源会自动收敛到不复权
- K 线主图：K 线、BOLL、MA、EMA、策略信号；`策略` 信号默认开启，只有存在选中成功策略回测结果时才会在图表展示买入/卖出标记
- K 线副图：成交量、VOL、MACD、KDJ、RSI，最多同时开启 3 个副图指标
- 分时主图：价格线、均价线、昨收线、MA、EMA、BOLL
- 分时成交量区：成交量柱、VOL MA
- 分时副图：MACD、RSI，最多同时开启 2 个副图指标
- 交互：缩放、拖拽、十字线、tooltip
- 顶部工具栏：证券代码、自选、分时/K线、K 线周期/复权/日期范围、刷新、指标、做T、AI（测试中）、当前证券标题和 `更多` 菜单；`更多` 聚合数据源、代理、AI 设置、本地缓存导入导出、检查更新和启动检查更新，分时视图隐藏 K 线专属控件
- 帮助菜单：使用说明书、版本更新说明、检查更新、关于 Stock Monitor
- 状态栏：当前视图的数据源、证券代码、记录数或分时点数、最新行情、加载状态、更新状态
- 本地持久化：查询条件、指标开关与参数、策略偏好、自选股、做T测算、非敏感 AI connector 配置、网络代理、启动检查更新配置会保存到 `electron-store`；HTTP API key 独立走 main process 凭据仓库，`safeStorage` 不可用时只保留本次会话临时 key；历史 K 线缓存保存到本地应用数据目录，并可通过 JSON 备份文件导入导出

## AI 模型接入

- 接入方式：第一版支持一个当前启用的 HTTP provider connector；不会并发调用多个模型、做模型投票或自动比较模型结果。
- Provider 预设：内置 OpenAI-compatible、DeepSeek、MiniMax、智谱 GLM、通义千问/DashScope、Kimi/Moonshot、硅基流动/SiliconFlow、百川智能/Baichuan、火山方舟/Ark 和自定义 provider；预设会填入常用 base URL，并为 model 提供可搜索下拉选项和自定义输入；API key 支持输入框直接粘贴，也支持通过粘贴按钮读取剪贴板。
- HTTP provider：使用 OpenAI-compatible `chat/completions` 协议，优先发送 `stream: true` 并实时显示 SSE delta 或 provider token payload；provider 明确不支持 streaming 时回退为非流式最终结果，鉴权、网络、超时和响应格式错误不会静默重试。
- 手动触发：应用启动、行情刷新、日期切换、缓存任务完成、打开 AI 分析弹窗都不会自动发送上下文；只有用户点击测试连接或分析按钮才会请求 HTTP provider。
- 实时反馈：AI 分析弹窗会在生成过程中持续追加内容，支持取消当前请求；取消、新请求或切换分析场景后，旧请求的后续事件不会覆盖当前状态。
- 展示格式：默认 prompt 要求模型输出简体中文可读小节，不输出 JSON；如果 provider 仍返回 JSON 字符串，弹窗会将字段转成可读小节展示。
- 上下文范围：AI 分析弹窗默认只发送当前证券、视图模式、数据源、日期范围、最新行情摘要、启用指标、策略回测摘要和用户问题；不会默认发送自选股列表、其它股票样本或完整 K 线/分时数据数组。
- 输出保存：流式 partial output 只存在于当前弹窗状态，不写入本地备份、跨会话历史或请求历史。
- 使用场景：自然语言生成策略、AI 解读回测报告、AI 策略诊断、AI 参数优化助手、自然语言智能选股、策略多周期/多股票对比、市场环境识别、AI 每日复盘、新闻/公告 + K 线联合分析、实验性 AI/ML 涨跌预测。
- 边界：自然语言策略和参数优化只生成可审查草稿和验证计划；新闻/公告联合分析第一版只使用用户提供摘要，不自动抓取或伪造材料；涨跌预测只展示实验性概率/风险假设，不进入策略信号、交易动作或收益承诺。
...
分时指标弹窗按基础显示、主图指标、成交量指标、副图指标和高级指标分组，支持分时 MA/EMA/BOLL、VOL MA、量比、换手率、MACD、KDJ、RSI、委比、内外盘、资金流的开关和参数配置；分时 `B/S` 不再作为当前可配置指标。
...
K 线首页展示周期、复权和日期范围控制，主图支持 BOLL、MA、EMA 和选中策略信号标记，副图区展示成交量和相关指标。

K 线日期范围会随工作区设置保存。应用重启加载保存设置时，结束日期会自动调整为启动当天，开始日期按原日期窗口长度同步平移；用户在当前会话内手动输入历史日期后，刷新、缓存和策略回测都会继续使用该手动范围。
...
K 线指标弹窗管理 BOLL、MA、EMA、策略、VOL、MACD、KDJ、RSI 的开关和参数，并限制副图指标数量；K 线 `B/S` 不再作为当前可配置指标，`策略` 信号默认开启。
...
K 线历史回测面板用于比较候选策略在顶部 K 线日期范围内的程序化信号和历史表现排名；策略模板默认全选，用户可按需取消。模板会展示类型、基本逻辑和推荐程度，推荐程度只是模板元数据，不替代历史回测评分。K 线 `策略` 信号默认开启，只有存在选中成功策略回测结果时才会在图表展示标记。历史回测不代表未来表现，也不构成买卖建议或收益承诺。
...
- `策略`：策略信号开关默认开启，用于显示当前选中成功策略回测结果的买入/卖出标记
...
- 指标开关和参数会随工作区设置持久化，并兼容旧版 `enabledIndicators` 配置；旧版 `B/S` 设置会被安全忽略，不再恢复为可见指标。

指标开关行为：

- 关闭 `BOLL` 只隐藏 BOLL，不影响 K 线。
- 关闭 `VOL` 会移除成交量指标面板。
- 关闭 `策略` 会移除选中策略信号标记，不影响 K 线和策略回测结果列表。
- 未运行回测、回测失败或没有选中成功策略结果时，即使 `策略` 开启，图表也不会展示策略买入/卖出标记。
- 重复切换不会累加重复指标或重复标记。

本地 model 会为数据集计算 `BOLL` 和 `VOL MA` 等派生值；图表层通过 `klinecharts` 创建 `BOLL`、`VOL`、`MA`、`EMA`、`MACD`、`KDJ`、`RSI` 指标，并使用自定义 overlay 渲染已开启的选中策略信号标记。
...
- 副图：`MACD` 默认 12/26/9，`KDJ` 默认 9/3/3，`RSI` 默认 6/12/24，最多同时开启 2 个副图指标
- 高级指标：`量比`、`换手率`、`委比`、`内外盘`、`资金流` 默认关闭，缺少前置数据时展示不可用状态

分时指标设置：

- 分时指标弹窗分为基础显示、主图指标、成交量指标、副图指标和高级指标。
- 参数会校验范围，周期类参数为 `1-250` 整数；`MACD` 要求快线小于慢线。
- 分时自动刷新或指标参数应用后会在本地重新计算指标，不额外请求行情。
- 分时图按区域展示图例：主图显示价格、均价、昨收、MA、EMA、BOLL；成交量区显示成交量、VOL MA、量比、换手率；副图区显示 MACD/RSI/KDJ。
- tooltip 会按鼠标所在区域展示相关指标值；分时 `B/S` 不再作为当前可配置指标，也不再展示 B/S 图例或 tooltip 文案。
...
- 策略模板默认全选，用户可以按需取消选择；默认全选只代表候选模板默认参与比较，不代表投资建议或收益承诺。
...
- K 线 `策略` 信号默认开启；选中策略后，只有存在成功回测结果时，图表才会展示该策略的历史买入/卖出信号标记。
...
- 菜单入口：`帮助 -> 检查更新`
- 工具栏入口：顶部 `更多 -> 检查更新`
- 工具栏开关：顶部 `更多 -> 启动检查更新` 控制是否启动后检查更新
- 有新版本时提示下载
- 下载完成后提示重启安装
- 开发环境不真实更新，只走日志和状态流
...
- 分时指标计算，包括 MA、EMA、BOLL、VOL MA、MACD、KDJ、RSI、量比、换手率、委比、内外盘和资金流
...
- klinecharts Adapter 指标开关、参数变更和策略 overlay 不重复累加
- Canvas Timeshare Adapter 午间休市压缩、分时指标图例、tooltip 和动态副图区
